...
BEGIN
  v_customer_id := COALESCE(p_customer_id, public.customer_id());
  SELECT COALESCE(cw.balance, 0), COALESCE(cw.period_limit, 0), COALESCE(cw.period_used, 0), cw.period_ends_at
  ...
  RETURN QUERY SELECT 
    v_balance >= p_amount, v_balance, p_amount, v_period_limit, v_period_used,
    GREATEST(v_period_limit - v_period_used, 0), v_period_ends_at;
END;
$$;
...
DECLARE
  ...
  v_user_id UUID;
BEGIN
  IF p_amount <= 0 THEN RAISE EXCEPTION 'Charge amount must be positive'; END IF;
  BEGIN v_user_id := public.user_id(); EXCEPTION WHEN OTHERS THEN v_user_id := NULL; END;
  ...
  FROM public.credit_wallets cw
  WHERE cw.customer_id = p_customer_id FOR UPDATE;
  ...
  ) VALUES (p_customer_id, v_user_id, -p_amount, 'charge', p_action_code, p_reason, p_reference_id)
  RETURNING credit_transaction_id INTO v_transaction_id;
  UPDATE public.credit_wallets
  SET balance = v_new_balance, period_used = period_used + p_amount, updated_at = now()
...
BEGIN
  ...
  FROM public.credit_wallets cw
  WHERE cw.customer_id = p_customer_id FOR UPDATE;
  IF v_current_balance IS NULL THEN RAISE EXCEPTION 'Customer wallet not found'; END IF;
  v_new_balance := v_current_balance + p_amount;
  INSERT INTO public.credit_transactions (
    customer_id, user_id, amount, type, action_code, reason, reference_id
  ) VALUES (p_customer_id, v_user_id, p_amount, 'refund', p_action_code, p_reason, p_reference_id)
  RETURNING credit_transaction_id INTO v_transaction_id;
  UPDATE public.credit_wallets
  SET balance = v_new_balance, period_used = GREATEST(period_used - p_amount, 0), updated_at = now()
  WHERE credit_wallets.customer_id = p_customer_id;
...
DECLARE
  ...
  v_user_id UUID;
BEGIN
  SELECT COALESCE(st.monthly_credits, 0) INTO v_monthly_credits
  FROM public.customers c
  LEFT JOIN public.subscription_types st ON st.subscription_type_id = c.subscription_type_id
  ...
  INSERT INTO public.credit_transactions (
    customer_id, user_id, amount, type, action_code, reason, reference_id
  ) VALUES (p_customer_id, v_user_id, v_monthly_credits - v_current_balance, 'reset', p_action_code, p_reason, p_reference_id)
  RETURNING credit_transaction_id INTO v_transaction_id;
  UPDATE public.credit_wallets
  SET balance = v_monthly_credits, period_limit = v_monthly_credits, period_used = 0,
      period_starts_at = p_period_starts_at, period_ends_at = p_period_ends_at, updated_at = now()
  WHERE credit_wallets.customer_id = p_customer_id;
