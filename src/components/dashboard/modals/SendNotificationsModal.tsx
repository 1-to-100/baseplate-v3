  ...
  const handleSend = async (selectedNotificationId: string) => {
    ...
    if (!hasError) {
      try {
        ...
        const errorMessage =
          httpError.response?.data?.message || 'Failed to send notifications. Please try again.';
              ...
              <Autocomplete
                ...
                getOptionLabel={(option) => {
                  ...
                  return option.customer?.name
                    ? `${displayName} (${option.customer.name})`
                    : displayName;
                }}
                renderOption={(props, option) => {
                  ...
                  const { key, ownerState, ...otherProps } =
              ...
              <Select
                ...
                renderValue={(selected) => {
                  if (selected.length === 0) {
                    return (
                      <Typography
                        sx={{ color: 'var(--joy-palette-text-primary)', fontSize: '14px' }}
                      >
                        Select customers
                      </Typography>
                    );
                  ...
                  return (
                    <Typography sx={{ color: 'var(--joy-palette-text-primary)', fontSize: '14px' }}>
