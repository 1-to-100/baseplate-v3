...
export default function AddUserToTeamModal({
  ...
  const handleSave = useCallback(async () => {
    if (selectedTab === TabName.SELECT) {
      ...
      try {
        const promises = selectedUsers.map((userId) => addTeamMemberMutation.mutateAsync(userId));
        ...
        toast.error(error instanceof Error ? error.message : 'Failed to add users to the team');
      }
    } else {
      ...
      >
        ...
        <Tabs
          ...
          onChange={(event, newValue) =>
            handleTabChange(event as React.SyntheticEvent, newValue as string | null)
          }
          sx={{ mt: 2, mb: 2 }}
        >
          <TabList
