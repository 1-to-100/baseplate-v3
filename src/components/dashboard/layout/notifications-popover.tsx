...
export function NotificationsPopover({
  ...
  useInAppNotificationsChannel((payload) => {
    ...
    toast(
      ...
      </Stack>,
      {
        ...
        style: {
          backgroundColor:
            colorScheme === 'dark' ? 'var(--joy-palette-background-level1)' : 'white',
  ...
  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1 }) => getNotifications({ type: 'in_app', page: pageParam }),
    getNextPageParam: (lastPage: GetNotificationsResponse) => lastPage.meta.next,
    initialPageParam: 1,
  });
  ...
  const notifications = React.useMemo(() => data?.pages.flatMap((page) => page.data) || [], [data]);
