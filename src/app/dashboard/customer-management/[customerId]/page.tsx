...
const Customer: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['users', currentPage, debouncedSearchValue, sortColumn, sortDirection, customerId],
    queryFn: async () => {
    ...
    <Box sx={{ p: { xs: 2, sm: 'var(--Content-padding)' } }}>
      ...
      <Stack sx={{ mt: 4 }}>
        <Breadcrumbs separator={<BreadcrumbsSeparator />}>
          <BreadcrumbsItem href={paths.dashboard.customerManagement.list} type='start' />
      ...
      >
        ...
        <Box sx={{ flex: 0.7, mt: { xs: 4, sm: 2 } }}>
          ...
          >
            ...
            >
              <DotsIcon weight='bold' size={22} color='var(--joy-palette-text-secondary)' />
            </Button>
          </Box>
          ...
          <Stack spacing={2}>
            <Stack direction='row' spacing={2}>
              ...
              >
                {customerData?.owner?.firstName.slice(0, 45)}{' '}
                {customerData?.owner?.lastName.slice(0, 45)}
              </Typography>
            </Stack>
            ...
            <Stack direction='row' spacing={2}>
              ...
              <Stack direction='row' spacing={0.5} alignItems='center'>
                {isCSUsersLoading ? (
                  ...
                  customerSuccessUsers.length > 0 &&
                  customerSuccessUsers[0] ? (
                  <>
                    <Typography
                      level='body-sm'
                      fontWeight='300'
                        ...
                        title={
                          <Stack spacing={0.5}>
                            {customerSuccessUsers.slice(1).map((csUser) => (
                              <Typography key={csUser.id} level='body-xs' sx={{ color: 'inherit' }}>
                                {csUser.email}
                              </Typography>
                            ))}
                          </Stack>
