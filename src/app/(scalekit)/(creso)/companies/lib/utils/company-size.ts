...
export function parseCompanySizeRange(value: string): { min: number; max: number | null } {
  ...
  // Handle ranges like "1-10", "501-1000"
  const rangeMatch = cleaned.match(/^(\d{1,3}(?:,\d{3})*)\s*-\s*(\d{1,3}(?:,\d{3})*)$/);
  if (rangeMatch) {
  ...
  // Handle "+" ranges like "10,001+"
  const plusMatch = cleaned.match(/^(\d{1,3}(?:,\d{3})*)\+$/);
  if (plusMatch) {
  ...
  const singleMatch = cleaned.match(/^(\d{1,3}(?:,\d{3})*)$/);
  if (singleMatch) {
