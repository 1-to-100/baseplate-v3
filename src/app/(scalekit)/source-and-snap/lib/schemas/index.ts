...
export const createDeviceProfileOptionSchema = z.object({
  programmatic_name: z
    ...
    .regex(/^[a-z0-9_]+$/, 'Programmatic name must contain only lowercase letters, numbers, and underscores'),
  display_name: z
    .string()
...
export const createCaptureSchema = z.object({
  ...
  screenshot_width: z.number().int('Screenshot width must be an integer').nonnegative('Screenshot width must be non-negative').nullable().optional(),
  screenshot_height: z.number().int('Screenshot height must be an integer').nonnegative('Screenshot height must be non-negative').nullable().optional(),
  screenshot_size_bytes: z.number().int('Screenshot size must be an integer').nonnegative('Screenshot size must be non-negative').nullable().optional(),
  html_size_bytes: z.number().int('HTML size must be an integer').nonnegative('HTML size must be non-negative').nullable().optional(),
  raw_html: z.string().nullable().optional(),
  raw_css: z.string().nullable().optional(),
  css_size_bytes: z.number().int('CSS size must be an integer').nonnegative('CSS size must be non-negative').nullable().optional(),
