  ...
  const run = useMemo(
    () => async () => {
      ...
      // Option tables (system scope)
      {
        ...
        log({
          name: 'typography_style_options',
          ok: typo.ok,
          detail: typo.ok ? `${typo.data.length} options` : typo.error,
        });
        log({
          name: 'logo_type_options',
          ok: logoTypes.ok,
          detail: logoTypes.ok ? `${logoTypes.data.length} options` : logoTypes.error,
        });
        log({
          name: 'font_options',
          ok: fonts.ok,
          detail: fonts.ok ? `${fonts.data.length} fonts (sample)` : fonts.error,
        });
        log({
          name: 'social_template_types',
          ok: stt.ok,
          detail: stt.ok ? `${stt.data.length} types` : stt.error,
        });
      ...
      try {
        ...
        log({
          name: 'visual_style_guides.create',
          ok: created.ok,
          detail: created.ok ? String(created.data.visual_style_guide_id) : String(created.error),
        });
        ...
        log({
          name: 'visual_style_guides.list',
          ok: listed.ok,
          detail: listed.ok ? `${listed.data.length} rows` : listed.error,
        });

        // Update VSG
        if (!vsgId) throw new Error('VSG ID is required');
        const updated = await updateVisualStyleGuide(vsgId, {
          description: 'Updated by diagnostics',
        });
        log({
          name: 'visual_style_guides.update',
          ok: updated.ok,
          detail: updated.ok ? 'ok' : updated.error,
        });

        // Palette color
        const pc = await createPaletteColor({
          hex: '#3366FF',
          usage_option: 'primary',
          sort_order: 1,
        });
        log({
          name: 'palette_colors.create',
          ok: pc.ok,
          detail: pc.ok ? String(pc.data.palette_color_id) : String(pc.error),
        });
        if (pc.ok) {
        ...
        log({
          name: 'palette_colors.list',
          ok: pcl.ok,
          detail: pcl.ok ? `${pcl.data.length} rows` : pcl.error,
        });
        if (paletteColorId) {
        ...
        if (firstRole) {
          ...
          log({
            name: 'typography_styles.create',
            ok: ts.ok,
            detail: ts.ok ? String(ts.data.typography_style_id) : String(ts.error),
          });
          if (ts.ok) typoId = String(ts.data.typography_style_id) as string | null;
          const tsl = await listTypographyStyles(vsgId!);
          log({
            name: 'typography_styles.list',
            ok: tsl.ok,
            detail: tsl.ok ? `${tsl.data.length} rows` : tsl.error,
          });
          if (typoId) {
            const tsu = await updateTypographyStyle(typoId, { font_weight: '600' });
            log({
              name: 'typography_styles.update',
              ok: tsu.ok,
              detail: tsu.ok ? 'ok' : tsu.error,
            });
          }
        }
        ...
        if (firstLogoType) {
          ...
          log({
            name: 'logo_assets.create',
            ok: la.ok,
            detail: la.ok ? String(la.data.logo_asset_id) : String(la.error),
          });
          if (la.ok) logoId = String(la.data.logo_asset_id) as string | null;
          const lal = await listLogoAssets(vsgId!);
          log({
            name: 'logo_assets.list',
            ok: lal.ok,
            detail: lal.ok ? `${lal.data.length} rows` : lal.error,
          });
          if (logoId) {
        ...
        if (firstType) {
          ...
          log({
            name: 'social_templates.create',
            ok: stc.ok,
            detail: stc.ok ? String(stc.data.social_template_id) : String(stc.error),
          });
          if (stc.ok) tmplId = String(stc.data.social_template_id) as string | null;
          const stl = await listSocialTemplates(vsgId!);
          log({
            name: 'social_templates.list',
            ok: stl.ok,
            detail: stl.ok ? `${stl.data.length} rows` : stl.error,
          });
          if (tmplId) {
