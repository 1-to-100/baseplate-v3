...
import { MagicWand as MagicWandIcon, Plus as PlusIcon, Sparkle as SparkleIcon } from '@phosphor-icons/react/dist/ssr';
import { 
...
export function CreateVisualStyleGuideWizard(): React.JSX.Element {
  ...
  const handleScanUrl = React.useCallback(async () => {
    ...
    try {
      ...
      if (colorsResult.status === 'rejected' || colorsResult.value?.error) {
        const error = colorsResult.status === 'rejected' ? colorsResult.reason : colorsResult.value.error;
      ...
      if (errors.length > 0) {
        ...
        toast.warning(`Visual style guide created, but some extractions failed. Check the guide for details.`);
      } else {
  ...
  const handleCreateGuide = React.useCallback(async (isFromScan = false) => {
    if (!name.trim()) {
      toast.error('Please enter a name for the style guide');
      return;
    }
          ...
          for (const typographyOption of typographyOptions) {
            try {
              await createTypographyStyle.mutateAsync({
                ...
                font_fallbacks: null,
                font_size_px: method === 'default' ? getDefaultFontSize(String(typographyOption.programmatic_name || '')) : 16,
                line_height: method === 'default' ? getDefaultLineHeight(String(typographyOption.programmatic_name || '')) : null,
                font_weight: method === 'default' ? getDefaultFontWeight(String(typographyOption.programmatic_name || '')) : null,
                color: null,
                css_snippet: null,
              ...
              console.warn(`Failed to create typography style for ${typographyOption.display_name}:`, error);
            }
          }
        }
      ...
      <Card sx={{ p: 4 }}>
        <Stack spacing={3} sx={{ alignItems: 'center' }}>
          ...
          <Typography level="title-lg">
            {step === 'crawling' ? 'Scanning website and extracting brand identity...' : 'Creating visual style guide...'}
          </Typography>
          {step === 'crawling' && (
