...
export async function listLogoAssets(visualStyleGuideId?: Id, params: ListParams = {}): Promise<ApiResult<LogoAsset[]>> {
...
export async function createLogoAsset(input: Omit<NewLogoAsset, 'logo_asset_id' | 'customer_id' | 'created_at'>): Promise<ApiResult<LogoAsset>> {
  const supabase = createClient();
  const { customerId } = await getContext();
  if (!customerId) return { ok: false, error: 'No customer context' };
...
export async function updateLogoAsset(id: Id, input: Partial<UpdateLogoAsset>): Promise<ApiResult<LogoAsset>> {
  const supabase = createClient();
  const { customerId } = await getContext();
  if (!customerId) return { ok: false, error: 'No customer context' };
...
export async function deleteLogoAsset(id: Id): Promise<ApiResult<{ deleted: boolean }>> {
  ...
  const { customerId } = await getContext();
  if (!customerId) return { ok: false, error: 'No customer context' };
  const { error } = await supabase.from('logo_assets').delete().eq('logo_asset_id', id).eq('customer_id', customerId);
  if (error) return { ok: false, error: error.message };
