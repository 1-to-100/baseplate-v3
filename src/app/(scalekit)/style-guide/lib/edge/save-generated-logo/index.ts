...
import {
  ...
  type SaveGeneratedLogoRequest,
  type SaveGeneratedLogoResponse,
  type PresetLogo,
} from './schema.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const body = parseSaveGeneratedLogoRequest(rawBody);
    const {
    ...
    const filteredLogoTypes =
      logo_type_option_ids && logo_type_option_ids.length > 0
    ...
    console.log(
      `Found ${filteredLogoTypes.length} logo types to update (filtered from ${logoTypes.length} total)`
    );
    ...
    for (const logoType of filteredLogoTypes) {
      ...
      console.log(
        `Processing logo type: ${logoType.display_name || logoType.programmatic_name} (${logoTypeOptionId})`
      );
    ...
    console.log(
      `Created/updated ${logoAssetIds.length} logo assets (all pointing to same shared file)`
    );
