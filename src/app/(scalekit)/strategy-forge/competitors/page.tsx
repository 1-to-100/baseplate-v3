...
function CreateCompetitorDialog({ open, onClose }: CreateCompetitorDialogProps): React.ReactElement {
...
export default function CompetitorsPage(): React.ReactElement {
  ...
  const [signalDialog, setSignalDialog] = React.useState<{ open: boolean; id: string | null; name: string | null }>(
    { open: false, id: null, name: null }
  );
  const [generatedCompetitors, setGeneratedCompetitors] = React.useState<GeneratedCompetitorDraft[]>([]);
  ...
  const upsertGeneratedCompetitor = (id: string, changes: Partial<Omit<GeneratedCompetitorDraft, "id">>) => {
    setGeneratedCompetitors((prev) => prev.map((entry) => (entry.id === id ? { ...entry, ...changes } : entry)));
  };
  ...
  const handleGenerateCompetitors = React.useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Unexpected error generating competitors.";
  ...
  const handleSaveGeneratedCompetitors = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Failed to save generated competitors.";
      toast.error(message);
    } finally {
  ...
  const handleBulkAction = async (action: string | null) => {
    ...
    if (action === "mark_monitored") {
      const monitoredStatus = statuses?.find(
        (status) => status.programmatic_name === "monitored"
      );
    ...
    if (action === "export") {
      ...
      if (rows && rows.length > 0) {
        const csv = ["Name,Category,Status,Source"].concat(
          rows.map((row) => `${row.name},${row.category ?? ""},${row.status_id ?? ""},${row.source_id ?? ""}`)
        );
          ...
          <Stack spacing={3}>
            <Stack spacing={1}>
              ...
              <Typography level="body-sm" color="neutral">
                Maintain an up-to-date register of competitors, sources, and signals to anchor strategy reviews.
              </Typography>
            </Stack>
            ...
            <Stack spacing={2}>
              ...
              <Typography level="body-sm" color="neutral">
                Use AI to propose a list of competitors. Review, edit, then save them into your register.
              </Typography>
              <Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
                <Button
                  variant="solid"
                        ...
                        <Stack spacing={1.5}>
                          <Stack direction={{ xs: "column", md: "row" }} spacing={1.5}>
                            <FormControl required sx={{ flex: 1 }}>
                              ...
                              <Input
                                value={entry.name}
                                onChange={(event) => upsertGeneratedCompetitor(entry.id, { name: event.target.value })}
                            ...
                            <FormControl sx={{ flex: 1 }}>
                              ...
                              <Input
                                ...
                                onChange={(event) =>
                                  upsertGeneratedCompetitor(entry.id, { websiteUrl: event.target.value })
                                }
                                placeholder="https://"
                              />
                            </FormControl>
                          ...
                          <FormControl>
                            ...
                            <Textarea
                              ...
                              onChange={(event) =>
                                upsertGeneratedCompetitor(entry.id, { description: event.target.value })
                              }
            ...
            <Stack direction={{ xs: "column", md: "row" }} spacing={1} alignItems={{ xs: "stretch", md: "center" }}>
              <FormControl sx={{ width: { xs: "100%", md: 220 } }}>
                ...
                <Select
                  ...
                  onChange={(_, value) => handleBulkAction(typeof value === 'string' ? value : null)}
            ...
            {isLoading ? (
              ...
              <Table hoverRow size="sm" aria-label="Competitor list">
                <thead>
                  <tr>
                    <th style={{ width: 48 }}>
                      <Checkbox
                        checked={competitors.every((competitor) => selectedIds.has(competitor.competitor_id))}
                        indeterminate={
                          selectedIds.size > 0 &&
                          selectedIds.size < (competitors?.length ?? 0)
                        }
                    ...
                    <tr key={competitor.competitor_id}>
                      <td>
                        <Checkbox
                          ...
                          onChange={(event) => toggleSelection(competitor.competitor_id, event.target.checked)}
                      ...
                      <td>
                        <Stack direction="row" spacing={1} alignItems="center">
                          ...
                          <IconButton
                            ...
                            variant="plain"
                            color="primary"
                            onClick={() => router.push(`/strategy-forge/competitors/${competitor.competitor_id}`)}
                      ...
                      <td>
                        {statuses?.find((status) => status.option_id === competitor.status_id)?.display_name ??
                          "Unknown"}
                      </td>
                      <td>
                        {dataSources?.find((source) => source.option_id === competitor.source_id)?.display_name ??
                          "—"}
                      </td>
                      ...
                      <td>
                        <Stack direction="row" spacing={1} justifyContent="flex-end">
                          ...
                          <IconButton
                            ...
                            variant="outlined"
                            color="danger"
                            onClick={() => deleteCompetitor.mutateAsync({ competitorId: competitor.competitor_id })}
              ...
              <Stack spacing={2} alignItems="flex-start">
                ...
                {isGeneratingCompetitors ? <LinearProgress variant="soft" sx={{ width: 240 }} /> : null}
              </Stack>
            )}
          </Stack>
        </CardContent>
