...
export default function PersonasListPage(): React.JSX.Element {
    ...
    const checkCustomerInfo = async () => {
      ...
      try {
        ...
        const { data: customerIdResult, error: customerIdError } =
          await supabase.rpc('customer_id');
        if (customerIdError) {
  ...
  const handleSuggestPersonas = React.useCallback(async () => {
    ...
    try {
      ...
      const personasWithIds = data.personas.map(
        (p: { name: string; description: string }, idx: number) => ({
          ...p,
  ...
  const handleUpdateSuggestion = (tempId: string, field: 'name' | 'description', value: string) => {
    setSuggestedPersonas(
      suggestedPersonas.map((p) => (p.tempId === tempId ? { ...p, [field]: value } : p))
    );
  };
  ...
  return (
    <Box sx={{ p: 'var(--Content-padding)' }}>
          ...
          <CardContent>
            ...
            >
              ...
              <Stack direction='row' spacing={2}>
                ...
                <Button startDecorator={<PlusIcon />} onClick={() => setIsCreateDialogOpen(true)}>
                  Create Persona
                </Button>
              </Stack>
            </Stack>
        ...
        {totalPages > 1 && (
          <Card>
            <CardContent>
              ...
              >
                <Typography level='body-sm'>
                  Showing {(page - 1) * limit + 1} to {Math.min(page * limit, personas.length)} of{' '}
      ...
      <Modal
        ...
        onClose={() => !isCreatingPersonas && setIsSuggestModalOpen(false)}
      >
        <ModalDialog
          sx={{ minWidth: '80vw', maxWidth: '1200px', maxHeight: '90vh', overflow: 'auto' }}
        >
          <ModalClose disabled={isCreatingPersonas} />
          <Typography level='h3'>Suggested Personas</Typography>
          <Typography level='body-sm' sx={{ mb: 2 }}>
            Review and edit the AI-generated persona suggestions. Add new suggestions or remove
            unwanted ones before creating.
          </Typography>
          ...
          {isLoadingSuggestions ? (
            ...
            >
              ...
              <Typography level='body-md'>
                Analyzing your company website to generate personas...
              </Typography>
            ...
            >
              ...
              <Typography level='body-md'>
                Creating{' '}
                {suggestedPersonas.filter((p) => p.name.trim() && p.description.trim()).length}{' '}
                personas...
              </Typography>
              <Typography level='body-sm' color='neutral'>
                Please wait while we create all personas in parallel
              </Typography>
            </Box>
            ...
            <Stack spacing={2}>
                    ...
                    <tr key={persona.tempId}>
                      <td>
                        <Input
                          value={persona.name}
                          onChange={(e) =>
                            handleUpdateSuggestion(persona.tempId, 'name', e.target.value)
                          }
                      ...
                      <td>
                        <Textarea
                          ...
                          onChange={(e) =>
                            handleUpdateSuggestion(persona.tempId, 'description', e.target.value)
                          }
              ...
              <Stack direction='row' spacing={2} sx={{ justifyContent: 'space-between' }}>
                ...
                <Button
                  ...
                  disabled={
                    suggestedPersonas.filter((p) => p.name.trim() && p.description.trim())
                      .length === 0
                  }
                >
                  Create{' '}
