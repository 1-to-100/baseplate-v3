...
function filterChangeLogs(logs: ReturnType<typeof useStrategyChangeLogsQuery>["data"], filters: FiltersState) {
  if (!logs) return [];
...
export default function StrategyChangeLogPage(): React.ReactElement {
  ...
  const filteredLogs = React.useMemo(() => filterChangeLogs(changeLogs, filters), [changeLogs, filters]);
  ...
  const handleExport = () => {
    ...
    const rows = filteredLogs.map((log) => {
      const type = changeTypes?.find((option) => option.option_id === log.change_type_id)?.display_name ??
        log.change_type_id;
      return [
  ...
  const handleRevert = (changeLogId: string) => {
    void changeLogId;
    alert("Revert workflow is not yet implemented. Capture this change log ID for manual assistance.");
  };
        ...
        <CardContent>
          <Stack spacing={1}>
            ...
            <Typography level="body-sm" color="neutral">
              Audit trail of drafts, edits, and publications. Use filters to isolate the entries you need.
            </Typography>
          </Stack>
        </CardContent>
      </Card>
            ...
            <Table size="sm" hoverRow aria-label="Strategy change log">
              ...
              <tbody>
                {filteredLogs.map((log) => {
                  const type = changeTypes?.find((option) => option.option_id === log.change_type_id);
                  return (
