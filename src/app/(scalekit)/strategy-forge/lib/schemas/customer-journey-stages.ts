...
export const createCustomerJourneyStageSchema = z.object({
  ...
  graduation_criteria: z.string().min(1, 'Graduation criteria is required').max(1000, 'Graduation criteria must be less than 1000 characters'),
...
export const updateCustomerJourneyStageSchema = z.object({
  ...
  graduation_criteria: z.string().min(1, 'Graduation criteria is required').max(1000, 'Graduation criteria must be less than 1000 characters').optional(),
  order_index: z.number().int().min(0).max(32767).optional().nullable(),
  code: z.string().max(50, 'Code must be less than 50 characters').optional().nullable(),
});
