...
import { useUserForm, validateEmail, type UserFormData } from "../user-form-fields/useUserForm";
...
export function AddUser({
  ...
  const getCustomerName = useCallback((id: string): string | undefined => {
    if (!customers) return undefined;
  ...
  const customerOptions = useMemo(() => {
    return customers?.sort((a, b) => a.name.localeCompare(b.name)).map((customer) => customer.name) || [];
  }, [customers]);
  ...
  useEffect(() => {
    if (teamId && teams.length > 0) {
      ...
      if (teamExists && formData.team !== teamIdString) {
        ...
        console.warn(`Team ${teamIdString} not found in teams list, but setting it anyway`, { teams, teamId: teamIdString });
        updateField("team", teamIdString);
      }
  ...
  const createUserMutation = useMutation({
    ...
    onSuccess: async (newUser: any) => {
      ...
      const emailSent = (newUser as any)?.emailSent !== false; // Default to true for backward compatibility
      const emailError = (newUser as any)?.emailError;
      ...
      if (!targetTeamId) {
        console.error("Cannot add user to team: missing teamId", { formDataTeam: formData.team, teamIdProp: teamId });
        toast.error("User created successfully, but team assignment failed: team information is missing.");
        resetForm();
        onSuccess();
        return;
      }
  ...
  }, [formData, effectiveCustomerId, isCurrentUserSystemAdmin, validateForm, setFormErrors, getRoleId, createUserMutation]);
  ...
  const handleCustomerChange = useCallback((newValue: string | null) => {
  ...
  const handleRoleChange = useCallback((newValue: string | null) => {
    updateField("role", newValue || "");
  }, [updateField]);

  const handleTeamChange = useCallback((newValue: string) => {
