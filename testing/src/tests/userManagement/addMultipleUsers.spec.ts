...
test.describe('Create multiple users', () => {
  ...
  test.beforeEach(async ({ page, request }) => {
    ...
    await test.step('Create new role for test', async () => {
      roleName = 'Role ' + Date.now();
      const apiKey = await apiMethods.getAccessToken(admin);
      const roleId = await apiMethods.createRole('New role for autotest', roleName, apiKey);
      await expect(
  ...
  test('Multi adding new users as Customer Success', async () => {
  ...
  test('Multi adding new users as User with All Permissions', async () => {
