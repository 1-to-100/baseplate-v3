...
test.describe('Notification details', () => {
  ...
  let initialNotificationText: string;
  let updatedNotificationTitle: string;
  let updatedNotificationText: string;
  ...
  test('Check notification details as System Administrator', async () => {
    await test.step('Login to app as admin', async () => {
      await loginPage.login(admin);
    });
    ...
    await test.step('Select "Open detail" action', async () => {
      await commonPage.selectAction(appData.actions.openDetail);
    });

    await test.step('Verify notification details modal title and all data fields', async () => {
      await expect(userManagementPage.notificationDetailsTitle).toHaveText(notificationData.notificationDetails);
      await expect(userManagementPage.notificationDetailsData(notificationData.notificationTable.title)).toContainText(
    ...
    await test.step('Close notification details modal', async () => {
      await userManagementPage.closeNotificationDetailsModal();
    });

    await test.step('Open more menu for created notification again', async () => {
      await commonPage.openMoreMenu(initialNotificationTitle);
    });
    ...
    await test.step('Open more menu for updated notification', async () => {
      await commonPage.openMoreMenu(updatedNotificationTitle);
    });

    await test.step('Select "Open detail" action for updated notification', async () => {
      await commonPage.selectAction(appData.actions.openDetail);
    });

    await test.step('Verify updated notification details modal title and all data fields', async () => {
      await expect(userManagementPage.notificationDetailsTitle).toHaveText(notificationData.notificationDetails);
      await expect(userManagementPage.notificationDetailsData(notificationData.notificationTable.title)).toContainText(
    ...
    await test.step('Close updated notification details modal', async () => {
      await userManagementPage.closeNotificationDetailsModal();
    });
  });
  ...
  test.skip('Check notification details as Customer Success', async () => {
    ...
    await test.step('Select notification type IN_APP and info channel', async () => {
      await commonPage.selectValueInDropdown(
        addNotificationModal.typeDropdown,
        UserPageHelper.toConstantCase(notificationData.notificationTypes.inApp),
      );
